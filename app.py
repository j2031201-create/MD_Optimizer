...
def calc_profit(rent, area, price):
...
if not df_all.empty:
    ...
    avg_rent  = _mean('월세_만원')
    avg_yield = _mean('예상수익률_pct')
...
if "데이터 수집" in app_mode:
    ...
    st.markdown('<p class="page-sub">매물 텍스트를 붙여넣으면 AI가 핵심 데이터를 추출하고 구글 시트에 자동 저장합니다.</p>', unsafe_allow_html=True)
    ...
    with col_L:
        ...
        raw_text = st.text_area(
            "매물 텍스트 붙여넣기", height=240,
            placeholder="네이버 부동산, 직방, 상가정보 등에서 복사한 매물 설명 붙여넣기\n\n예시:\n위치: 강남구 역삼동 테헤란로\n면적: 25평 / 보증금: 5,000만원 / 월세: 350만원\n권리금: 3,000만원"
        )
    ...
    with col_R:
        ...
        # 기본 상태: 마지막 결과 또는 빈 안내
        if not btn_extract:
            if st.session_state.last_extracted:
                d = st.session_state.last_extracted
                pr = calc_profit(d.get('월세_만원'), d.get('면적_평'), d.get('권장분양가_만원'))
            ...
            try:
                ...
                if data:
                    ...
                    st.session_state.md_data = pd.concat(
                        [st.session_state.md_data, pd.DataFrame([data])], ignore_index=True
                    )
...
# ═══════════════════════════════════════════════════════
    ...
    with c1:
        reg_list = ["전체"] + sorted(df_all['지역그룹'].dropna().unique().tolist())
        sel_reg  = st.selectbox("지역 선택", reg_list)
    with c2:
        sec_list = ["전체"] + sorted(df_all['AI분류업종'].dropna().unique().tolist())
        sel_sec  = st.selectbox("업종 필터", sec_list)
    with c3:
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with ca:
            ...
            if not df_b.empty:
                fig = px.box(df_b, x="AI분류업종", y="월세_만원", color="AI분류업종",
                             title="업종별 월세 분포", color_discrete_sequence=PALETTE)
                fig.update_layout(**CHART_LAYOUT, showlegend=False)
                st.plotly_chart(fig, use_container_width=True)
        with cb:
            df_s = df.dropna(subset=['면적_평','월세_만원'])
            if not df_s.empty:
                fig2 = px.scatter(df_s, x="면적_평", y="월세_만원", color="AI분류업종",
                                  title="면적 vs 월세", hover_data=['상호명'], color_discrete_sequence=PALETTE)
    ...
    if do_ai and not df.empty:
        ...
        avg_r = df['월세_만원'].mean() if df['월세_만원'].notna().any() else 0
        avg_d = df['보증금_만원'].mean() if df['보증금_만원'].notna().any() else 0
        ...
        try:
            resp = model.generate_content(MD_RECOMMEND_PROMPT.format(
                region=sel_reg, avg_rent=f"{avg_r:.0f}", avg_deposit=f"{avg_d:.0f}",
...
# ═══════════════════════════════════════════════════════
    ...
    st.markdown('<p class="page-sub">임대 조건을 입력하면 적정 분양가와 수익률을 즉시 계산합니다.</p>', unsafe_allow_html=True)
    ...
    with co:
        ...
        rec_p    = int(annual / (target_yield / 100)) if target_yield > 0 else 0
        rent_pp  = monthly_rent / area if area > 0 else 0
        payback  = rec_p / annual if annual > 0 else 0
        ...
        <div class="profit-card" style="margin-top:0;">
          ...
          <div class="profit-row"><span>투자 회수 기간</span><span class="profit-num">{payback:.1f}년</span></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    fig_s.update_layout(
        title="수익률별 권장 분양가",
        **{k: v for k, v in CHART_LAYOUT.items()}, showlegend=False
    )
